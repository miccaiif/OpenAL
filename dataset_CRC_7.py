...
class CRC(torch.utils.data.Dataset):
    def __init__(self, label_flag=None, train=True, gen_cand=False):
        ...
        if train or gen_cand:
            self.all_patches_ori = np.load(os.path.join(save_path, "./training_datasets/embeddings.npy"))
            self.labels_ori = np.load(os.path.join(save_path, "./training_datasets/labels.npy"))
            self.all_patches = []
            self.labels = []
            for i in range(len(self.all_patches_ori)):
                if self.labels_ori[i] != 1 and self.labels_ori[i] != 2:
                    self.all_patches.append(self.all_patches_ori[i])
                    self.labels.append(self.labels_ori[i])
            self.all_patches = np.array(self.all_patches)
            self.labels = np.array(self.labels)
            self.class2index = np.load(os.path.join(save_path, "./training_datasets/class2index.npy"))
        else:
            # test pick data from unlabelled pool
            self.all_patches_ori = np.load(os.path.join(save_path, "./testing_datasets/embeddings.npy"))
            self.labels_ori = np.load(os.path.join(save_path, "./testing_datasets/labels.npy"))
            self.all_patches = []
            self.labels = []
            for i in range(len(self.all_patches_ori)):
                if self.labels_ori[i] != 1 or self.labels_ori[i] != 2:
                    self.all_patches.append(self.all_patches_ori[i])
                    self.labels.append(self.labels_ori[i])
            self.all_patches = np.array(self.all_patches)
            self.labels = np.array(self.labels)
            self.class2index = np.load(os.path.join(save_path, "./testing_datasets/class2index.npy"))
...
if __name__ == '__main__':
    train_dataloader, gen_can_dataloader, test_dataloader = load_dataset()
